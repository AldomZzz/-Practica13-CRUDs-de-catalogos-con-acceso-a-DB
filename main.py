...
""")
        ...
        self.busqueda_input.returnPressed.connect(self.buscar_y_seleccionar)
    ...
    def buscar_y_seleccionar(self):
        """Busca productos y selecciona automáticamente si hay uno exacto"""
        self.buscar_productos()
        
        # Si hay exactamente un resultado, lo seleccionamos
        if self.tabla_productos.rowCount() == 1:
            self.tabla_productos.selectRow(0)
            self.cantidad_input.setFocus()  # Movemos el foco al campo de cantidad

    def buscar_productos(self):
        try:
            cursor = self.conexion.cursor()
            busqueda = self.busqueda_input.text().strip()  # Limpiamos espacios
            ...
            for fila_idx, (id_art, nombre, precio, stock) in enumerate(productos):
                ...
                self.tabla_productos.setItem(fila_idx, 0, QTableWidgetItem(str(id_art)))
